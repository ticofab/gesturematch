...
myApp.controller('paramTable', ['$scope', '$log', function($scope, $log) {
    ...
    $scope.$watchCollection("[dev1lat,dev1lon,dev1ss,dev1se,dev1ep,dev1criteria,dev1groupId,dev1idInGroup]", function(nv) {
        $scope.match1json = createMatchJson(nv)
        $scope.groupMatch1json = createGroupMatchJson(nv, $scope.dev1groupId, parseInt($scope.dev1idInGroup))
    });

    $scope.$watchCollection("[dev2lat,dev2lon,dev2ss,dev2se,dev2ep,dev2criteria,dev2groupId,dev2idInGroup]", function(nv) {
        $scope.match2json = createMatchJson(nv)
        $scope.groupMatch2json = createGroupMatchJson(nv, $scope.dev2groupId, parseInt($scope.dev2idInGroup))
    });

    $scope.$watchCollection("[dev3lat,dev3lon,dev3ss,dev3se,dev3ep,dev3criteria,dev3groupId,dev3idInGroup]", function(nv) {
        $scope.match3json = createMatchJson(nv)
        $scope.groupMatch3json = createGroupMatchJson(nv, $scope.dev3groupId, parseInt($scope.dev3idInGroup))
    });

    $scope.$watchCollection("[dev4lat,dev4lon,dev4ss,dev4se,dev4ep,dev4criteria,dev4groupId,dev4idInGroup]", function(nv) {
        $scope.match4json = createMatchJson(nv)
        $scope.groupMatch4json = createGroupMatchJson(nv, $scope.dev4groupId, parseInt($scope.dev4idInGroup))
    });
    ...
    function onM1(evt) {
        ...
        if (evt.data != "k") {
            ...
            if (mjs.myId != undefined) {
                $scope.dev1idInGroup = mjs.myId;
            }
            $scope.$apply();
        }
    }
    ...
    $scope.match1 = function() {
        if (myWebSocket1 != undefined) {
          myWebSocket1.send($scope.match1json)
      }
    }
    $scope.match2 = function() {
        if (myWebSocket1 != undefined) {
          myWebSocket2.send($scope.match2json)
      }
    }
    $scope.match3 = function() {
        if (myWebSocket3 != undefined) {
          myWebSocket3.send($scope.match3json)
      }
    }
    $scope.match4 = function() {
        if (myWebSocket4 != undefined) {
          myWebSocket4.send($scope.match4json)
      }
    }
    ...
    $scope.groupcom1 = function() {myWebSocket1.send($scope.groupMatch1json)}
    $scope.groupcom2 = function() {myWebSocket2.send($scope.groupMatch2json)}
    $scope.groupcom3 = function() {myWebSocket3.send($scope.groupMatch3json)}
    $scope.groupcom4 = function() {myWebSocket4.send($scope.groupMatch4json)}
